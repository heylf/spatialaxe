...
def main():
    ...
    if os.path.exists(f"{output_path}/{output_folder}"):
        shutil.rmtree(f"{output_path}/{output_folder}")
    os.makedirs(f"{output_path}/{output_folder}")

    # Copy the entire reference bundle as is
    for root, _, files in os.walk(raw_bundle):
        rel_path = os.path.relpath(root, raw_bundle)
        target_path = os.path.join(f"{output_path}/{output_folder}", rel_path)
    ...
    for category in ["points", "shapes", "tables"]:
        category_path = os.path.join(f"{output_path}/{output_folder}", category)
    ...
    for category in ["points", "shapes", "tables"]:
        ...
        output_category_path = os.path.join(f"{output_path}/{output_folder}", category)
